...
- `src/dashboard`: Internal Astro dashboard (behind SWA EasyAuth); screens live in `src/pages` as thin wrappers that mount Svelte island components from `src/components` via `client:*` directives.
...
- `.slnx` solution: use `alpakasoelde.slnx` to open the API projects together; `global.json` pins .NET SDK 10.0.0.
...
- Tests: no .NET test projects exist in the solution (the former `dashboard-api.Tests`/`website-api.Tests` were removed from `alpakasoelde.slnx`); verification relies on `dotnet build` for both APIs, `pnpm run build` (runs `astro check`) in both Astro projects, and the REST samples in each `requests.http`. Reintroduce and register test projects before adding unit tests.
...
- Dashboard UI: whenever an icon is needed, use the installed `@lucide/svelte` package instead of introducing other icon sources.

## Astro & Svelte Islands (website & dashboard)
- `src/website` uses `.astro` components exclusively and ships zero client JS by default; do not introduce React, Vue, Svelte, or other framework components there.
- `src/dashboard` mounts Svelte islands: keep `src/pages/*.astro` as thin wrappers around Svelte components loaded with `client:*` directives (mostly `client:only="svelte"`); do not add other framework islands.
...
- No coverage target exists; add unit tests only after reintroducing test projects (see Build, Test, and Development Commands).
...
- Table usage: `alpakas`, `events`, `messages`, `gutscheine`, and `pageviews` tables with partition keys `AlpakaPartition` (alpakas), `ContactPartition` (messages), `GutscheinePartition` (gutscheine), `Pv|{yyyy-MM-dd}` (pageviews), and AlpakaId per row for events; storage is provisioned via `infrastructure/modules/storage.bicep` (adopted in place).
