...
- [ ] Write the plan (`docs/plans/013-dashboard-assistant.md`)
- [ ] Confirm Azure OpenAI quota for a second deployment in `germanywestcentral`
- [ ] `infrastructure`: parameterise `openai.bicep` for a second deployment, wire into `main.bicep`/`main.bicepparam`, `az bicep build` + `what-if`
- [ ] Key Vault secrets + `DASHBOARD_KEYS` in `sync-swappsettings.sh`
- [ ] `dashboard-api`: `shared/EnvironmentVariables.cs`, `local.settings.json` placeholders, package references
- [ ] `dashboard-api`: `features/assistant/AssistantTools.cs` + `Assistant.cs`, registration in `Program.cs`
- [ ] `dashboard-api-tests`: fake `IChatClient`, tool-dispatch and guard-rail tests
- [ ] `dashboard`: `AssistantChat.svelte`, `src/pages/assistent.astro`, navbar entry
- [ ] Extend `src/dashboard-api/requests.http`
- [ ] Verify builds/checks/tests; deploy dashboard SWA
...
Packages added to `src/dashboard-api/dashboard-api.csproj`: `Microsoft.Agents.AI` 1.20.0, `Microsoft.Agents.AI.OpenAI` 1.20.0, `Microsoft.Extensions.AI` 10.9.0, `Microsoft.Extensions.AI.OpenAI` 10.9.0, `OpenAI` 2.13.0 (the last three arrive transitively but are pinned explicitly, because the `Azure.AI.OpenAI` conflict above makes the exact `OpenAI` version load-bearing). Key-based auth matches the rest of the estate (`OpenAiApiKey` in Key Vault); switching this account to managed identity would be an improvement, but it is a separate change touching `website-api` too.
