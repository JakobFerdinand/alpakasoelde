...
public sealed class GetMessages
{
	...
	public sealed record DashboardMessage(string Id, string Name, string Email, string Phone, string Message, DateTimeOffset? Timestamp, bool IsSpam);
	...
	public sealed class Handler(IReadStore store)
	{
		...
		public async Task<IReadOnlyList<DashboardMessage>> HandleAsync(Query query, CancellationToken cancellationToken)
		{
			...
			return messages
				...
				.Select(m => new DashboardMessage(m.RowKey, m.Name, m.Email, m.Phone ?? string.Empty, m.Message, m.Timestamp, m.IsSpam))
				.ToList();
		}
	}
}
