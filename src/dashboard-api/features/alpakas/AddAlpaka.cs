...
public sealed record AddAlpakaCommand(string Name, string Geburtsdatum, AlpakaImagePayload? Image);
...
public sealed record AddAlpakaResult(string Id, string Name, string Geburtsdatum, string? ImageUrl);

public sealed record AddAlpakaResponse(AddAlpakaResult? Result = null, IReadOnlyCollection<string>? ValidationErrors = null)
{
...
public sealed class AddAlpakaHandler(IAlpakaWriteStore writeStore, IAlpakaImageStore imageStore, ILogger<AddAlpakaHandler> logger)
{
	private readonly IAlpakaWriteStore _writeStore = writeStore;
	private readonly IAlpakaImageStore _imageStore = imageStore;
	private readonly ILogger<AddAlpakaHandler> _logger = logger;

	private const int NameMaxLength = 100;
	private const long MaxImageSizeBytes = 15 * 1024 * 1024;

	public async Task<AddAlpakaResponse> HandleAsync(AddAlpakaCommand command, CancellationToken cancellationToken)
	{
		List<string> errors = [];
		...
		if (errors.Count > 0)
		{
			return new AddAlpakaResponse(ValidationErrors: errors);
		}

		if (command.Name.Length > NameMaxLength)
		{
			return new AddAlpakaResponse(ValidationErrors: [
		...
		return new AddAlpakaResponse(new AddAlpakaResult(entity.RowKey, entity.Name, entity.Geburtsdatum, entity.ImageUrl));
	}
}

public sealed class AddAlpakaFunction(AddAlpakaHandler handler, ILogger<AddAlpakaFunction> logger)
{
	private readonly AddAlpakaHandler _handler = handler;
	private readonly ILogger<AddAlpakaFunction> _logger = logger;
		...
		[HttpTrigger(AuthorizationLevel.Function, "post", Route = "alpakas")] HttpRequestData req)
	{
		...
		AddAlpakaResponse response = await _handler.HandleAsync(
			new AddAlpakaCommand(name ?? string.Empty, geburtsdatum ?? string.Empty, imagePayload),
