...
public sealed record GetAlpakasQuery;
...
public sealed class GetAlpakasHandler(IAlpakaReadStore readStore, IImageUrlSigner imageSigner)
{
	private readonly IAlpakaReadStore _readStore = readStore;
	private readonly IImageUrlSigner _imageSigner = imageSigner;

	public async Task<IReadOnlyList<AlpakaListItem>> HandleAsync(GetAlpakasQuery query, CancellationToken cancellationToken)
	{
...
public sealed class GetAlpakasFunction(GetAlpakasHandler handler, ILogger<GetAlpakasFunction> logger)
{
	private readonly GetAlpakasHandler _handler = handler;
	private readonly ILogger<GetAlpakasFunction> _logger = logger;

	[Function("get-alpakas")]
	public async Task<HttpResponseData> Run(
		[HttpTrigger(AuthorizationLevel.Function, "get", Route = "alpakas")] HttpRequestData req)
	{
		IReadOnlyList<AlpakaListItem> alpakas = await _handler.HandleAsync(new GetAlpakasQuery(), req.FunctionContext.CancellationToken);
