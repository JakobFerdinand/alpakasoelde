...
public sealed record UpdateAlpakaCommand(string Id, string Name, string Geburtsdatum, AlpakaImagePayload? Image);

public sealed record UpdateAlpakaResult(string Id, string Name, string Geburtsdatum, string? ImageUrl);

public sealed record UpdateAlpakaResponse(UpdateAlpakaResult? Result = null, IReadOnlyCollection<string>? ValidationErrors = null, bool NotFound = false)
{
...
public sealed class UpdateAlpakaHandler(IAlpakaUpdateStore alpakaStore, IAlpakaImageReplacementStore imageStore, ILogger<UpdateAlpakaHandler> logger, IImageUrlSigner imageSigner)
{
    ...
    private readonly ILogger<UpdateAlpakaHandler> _logger = logger;
    private readonly IImageUrlSigner _imageSigner = imageSigner;

    private const int NameMaxLength = 100;

    public async Task<UpdateAlpakaResponse> HandleAsync(UpdateAlpakaCommand command, CancellationToken cancellationToken)
    {
        List<string> errors = [];
        ...
        if (errors.Count > 0)
        {
            return new UpdateAlpakaResponse(ValidationErrors: errors);
        }

        if (command.Name.Length > NameMaxLength)
        {
            return new UpdateAlpakaResponse(ValidationErrors: [$"Name exceeds {NameMaxLength} characters."]);
        }

        AlpakaEntity? existing = await _alpakaStore.GetAsync(command.Id, cancellationToken).ConfigureAwait(false);
        if (existing is null)
        {
            return new UpdateAlpakaResponse(NotFound: true);
        }
        ...
        if (command.Image is not null)
        {
            ...
            {
                return new UpdateAlpakaResponse(ValidationErrors: [ex.Message]);
            }
        }
        ...
        return new UpdateAlpakaResponse(new UpdateAlpakaResult(existing.RowKey, existing.Name, existing.Geburtsdatum, signedUrl));
    }
}

public sealed class UpdateAlpakaFunction(UpdateAlpakaHandler handler, ILogger<UpdateAlpakaFunction> logger)
{
    private readonly UpdateAlpakaHandler _handler = handler;
    private readonly ILogger<UpdateAlpakaFunction> _logger = logger;
        ...
        string alpakaId)
    {
        ...
        UpdateAlpakaResponse response = await _handler.HandleAsync(
            new UpdateAlpakaCommand(alpakaId, name ?? string.Empty, geburtsdatum ?? string.Empty, imagePayload),
