...
public sealed record SendMessageCommand(string Name, string Email, string Message, bool PrivacyAccepted);

public sealed record SendMessageResult(string RedirectLocation);
...
public sealed class SendMessageHandler(IMessageWriteStore store, IEmailSender emailSender, ILogger<SendMessageHandler> logger, IConfiguration configuration)
{
	...
	public async Task<(SendMessageResult? Result, ValidationProblem? Validation)> HandleAsync(SendMessageCommand command, CancellationToken cancellationToken)
	{
		...
		return (new SendMessageResult("/nachricht-gesendet"), null);
	}
}

public sealed class SendMessageFunction(SendMessageHandler handler, ILogger<SendMessageFunction> logger)
{
	private readonly SendMessageHandler _handler = handler;
	private readonly ILogger<SendMessageFunction> _logger = logger;
		...
		FunctionContext context)
	{
		...
		var (result, validation) = await _handler.HandleAsync(new SendMessageCommand(name ?? string.Empty, email ?? string.Empty, messageContent ?? string.Empty, privacyAccepted), req.FunctionContext.CancellationToken);
